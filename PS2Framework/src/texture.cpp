...
// Set the valid flag to false until texture loaded
CTexture::CTexture()
{
	m_bValid = false;
}
...
// This method will load 24 or 8 bit windows bmp files
bool CTexture::LoadBitmap(const char * strFilename, bool bPinkTransparent, bool bRedAsAlpha)
{
	// Open the file
	FILE * fp = NULL;
	...
	if(bmp.bitmapinfoheader.biBitCount == 8)
	{
		...
		{	
			...
			else
			{
				...
				if(clut[p] == 0xFF00FF && bPinkTransparent)
					pPtr[3] = 0x00;
				else
					pPtr[3] = 0x80;
			}
	...
	for(i = 0; i < (m_iWidth * m_iHeight); i++, iColumn++)
	{
		...
		else	// Must be 24 bit colour
		{
			...
			else
			{
				...
				if(pixel == 0xFF00FF && bPinkTransparent)
					pPtr[3] = 0;
				else
					pPtr[3] = 0x80;
			}
	...
	// Flag the texture as valid
	m_bValid = true;
	
	return true;
}

// Select the texture for use - it needs to be uploaded first
