...
class CSPS2Manager : public CSingleton<CSPS2Manager>
{
	public:
		...
		// This function takes a screen shot and saves it to a TGA file
		void ScreenShot(const int ImageType = 1);
	...
	private:
		...
		void DownloadVram(CDMAMem & DMAChainMem, CDMAMem & SShotMem, int addr, int bufw, int x, int y, int w, int h);
